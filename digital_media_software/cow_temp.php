<html><head><title>Project 4 Starter Kit:Cowpies 2</title>
</head><body><form method='post'>
<?php
// Cowpie: Project 4 Starter Kit: DIG 3134 - Fall 2011
// Moshell
// This version (cowpie2.php) contains "dummy code" for the control
// structures needed to store and retrieve previous shots, via a database.
...
{global $Imagewidth, $Imageheight;
	$vmax=500;
	if (!$velocity) $velocity=0; // so we can see a zero in the error message
	if (!$angle) $angle=0;		// ditto.
   ...
   //imageline($image, x1, 			y1, 		x2, 			y2, 		  	$colorGrey);
	...
	{
		imagesetthickness($image, 50);	
		imageline($image, 300, 200, 300,550, $colorPink); 
		imagesetthickness($image, 2);	
	}
...
function drawinputs()
{
	...
	print "<br />
		...
		<input type='text' size=2 name='velocity'>Velocity, 1 to 100 meters/sec <br />
		<input type='submit' name='action' value='Launch Cowpie!'> <br />
		<input type='submit' name='action' value='Show History'> <br />
		<input type='submit' name='action' value='Erase History'> <br />
		";
		
} # drawinputs

#storehistory: to be built later
function storehistory($angle,$velocity)
{
	print "<h3>function 'storehistory' is not yet implemented.</h3>";
}
#showhistory: dummy, to show the concept
function showhistory($image,$color)
{
	
	print "<h3>function 'showhistory' is showing fake data; database not yet implemented.</h3>";
	$image=drawtrajectory($image, 70,70,$color);
	$image=drawtrajectory($image, 45,45,$color);
	return $image;
}

#erasehistory: to be built later
function erasehistory()
{
	print "<h3>function 'drawhistory' is not yet implemented.</h3>";
} # erasehistory
